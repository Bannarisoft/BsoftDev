    ...
    public class NotificationsQueryHandler
    {
       ...
       public async Task<ApiResponseDTO<NotificationResponse>> Handle(NotificationRequest request, CancellationToken cancellationToken)
        ...
        if (string.IsNullOrWhiteSpace(request.Username))
         ...
         _logger.LogInformation("User {Username} found. Retrieving Password expiry details...", request.Username);
        ...
        {
            _logger.LogWarning("Password last change date not found for Username: {Username}", request.Username);
            return new ApiResponseDTO<NotificationResponse>
            {
                IsSuccess = true,
        ...
        _logger.LogInformation("Getting Last Password Change Date for Username: {Username}", lastPasswordChangeDate);
        ...
        pwdExpiryDays, pwdExpiryAlertDays);
        ...
        {
            _logger.LogWarning("Password has expired for Username: {Username}", request.Username);
            return new ApiResponseDTO<NotificationResponse>
            {
                IsSuccess=false,
        ...
        {
        _logger.LogInformation("Password is still valid for Username: {Username}", request.Username);
        return new ApiResponseDTO<NotificationResponse>
        {
            IsSuccess=false,

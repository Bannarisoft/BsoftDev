...
{
    public class GetModuleByIdQueryHandler : IRequestHandler<GetModuleByIdQuery,ApiResponseDTO<ModuleDto>>
    {
    ...
    public GetModuleByIdQueryHandler(IModuleQueryRepository moduleQueryRepository, IMapper mapper, IMediator mediator)
    {
        ...
        public async Task<ApiResponseDTO<ModuleDto>> Handle(GetModuleByIdQuery request, CancellationToken cancellationToken)
        {
            var result = await _moduleQueryRepository.GetModuleByIdAsync(request.Id);
             if (result == null)
            {
                return new ApiResponseDTO<ModuleDto> { IsSuccess = false, Message = "Module not found", Data = null };
            }
            var modules = _mapper.Map<ModuleDto>(result);
          ...
          return new ApiResponseDTO<ModuleDto> { IsSuccess = true, Message = "Success", Data = modules };
        }
    }
}
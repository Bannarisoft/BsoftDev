...
namespace BSOFT.Infrastructure.Repositories
{
    ...
    public class StateRepository : IStateRepository
    {
        ...
        {
              return await _applicationDbContext.States.AsNoTracking().FirstOrDefaultAsync(b => b.Id == id);
        }

        public async Task<List<States>> GetByStateNameAsync(string searchPattern)
        {
            return await _applicationDbContext.States
            .Where(c => c.StateName.Contains(searchPattern, StringComparison.OrdinalIgnoreCase) || c.StateCode.Contains(searchPattern) )

                ...
                switch (rule.Rule)
                {
                    case "NotEmpty":
                        RuleFor(x => x.ModuleId)
                            ...
                            .NotEmpty()
                            .WithMessage($"{nameof(UpdateWorkflowTypeCommand.ModuleTypeName)} {rule.Error}");
                        break;
                    case "AlreadyExists":
                        RuleFor(x => new { x.ModuleTypeName,x.ModuleId, x.Id })
                         .MustAsync(async (WorkflowType, cancellation) =>
                      !await _workflowTypeQuery.AlreadyExistsAsync(WorkflowType.ModuleTypeName,WorkflowType.ModuleId, WorkflowType.Id))

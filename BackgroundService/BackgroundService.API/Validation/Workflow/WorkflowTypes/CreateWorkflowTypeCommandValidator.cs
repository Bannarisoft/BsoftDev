    ...
    public class CreateWorkflowTypeCommandValidator : AbstractValidator<CreateWorkflowTypeCommand>
    {
        ...
        public CreateWorkflowTypeCommandValidator(MaxLengthProvider maxLengthProvider, IWorkflowTypeQuery workflowTypeQuery)
        {
               ...
               foreach (var rule in _validationRules)
            {
                ...
                {
                    case "NotEmpty":
                        RuleFor(x => x.ModuleTypeName)
                            .NotEmpty()
                            .WithMessage($"{nameof(CreateWorkflowTypeCommand.ModuleTypeName)} {rule.Error}");
                        break;
                    case "MaxLength":                        
                        RuleFor(x => x.ModuleTypeName)
                        ...
                        break;
                    case "AlreadyExists":                       
                        RuleFor(x => new { x.ModuleTypeName, x.ModuleId })
                          .MustAsync(async (WorkflowType, cancellation) => !await _workflowTypeQuery.AlreadyExistsAsync(WorkflowType.ModuleTypeName,WorkflowType.ModuleId))

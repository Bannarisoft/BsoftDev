...
public class DashboardQueryHandler : IRequestHandler<DashboardQuery, ChartDto>
{
    ...
    public async Task<ChartDto> Handle(DashboardQuery request, CancellationToken cancellationToken)
    {
        ...
        return request.Type.ToLower() switch
        {
            ...
            "maintenancehrs-dept" => await _repository.MaintenanceHoursAsync(request.FromDate, request.ToDate,request.Type),
            "maintenancehrs-machinegroup" => await _repository.MaintenanceHoursAsync(request.FromDate, request.ToDate, request.Type,request.DepartmentId),
            "maintenancehrs-machine" => await _repository.MaintenanceHoursAsync(request.FromDate, request.ToDate, request.DepartmentId, request.MachineGroupId),

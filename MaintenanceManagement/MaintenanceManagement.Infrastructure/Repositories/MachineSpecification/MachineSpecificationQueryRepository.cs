...
namespace MaintenanceManagement.Infrastructure.Repositories.MachineSpecification
{
    public class MachineSpecificationQueryRepository : IMachineSpecificationQueryRepository
    {
        ...
        public async Task<MachineSpecificationDto?> GetByIdAsync(int Id)
        {
            ...
            const string query = @"
                                ...
                                MS.IsActive 
                                FROM 
                                Maintenance.MachineSpecification MS INNER JOIN Maintenance.MachineMaster MC ON MS.MachineId=MC.Id
                                INNER JOIN 
                                Maintenance.MiscMaster MM ON MM.Id=MS.SpecificationId 
                                WHERE 
                                MS.IsDeleted=0 
                                AND MC.Id=@Id
                                AND MC.UnitId=@UnitId";

            var machineMaster = await _dbConnection.QueryFirstOrDefaultAsync<MachineSpecificationDto>(query, new { Id, unitId });
